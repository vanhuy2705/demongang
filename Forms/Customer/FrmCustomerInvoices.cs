...
{
    public FrmCustomerInvoices(){InitializeComponent();AppTheme.StyleGrid(grid);AppTheme.StyleSecondary(btnRefresh);AppTheme.StylePrimary(btnPay);btnRefresh.Click+=(_,__)=>LoadData();btnPay.Click+=(_,__)=>Pay();Shown+=(_,__)=>LoadData();}
    ...
    private void Pay(){if(grid.CurrentRow==null)return;var id=Convert.ToInt32(grid.CurrentRow.Cells["DatSanID"].Value);using var f=new FrmPayment(id);f.ShowDialog(this);LoadData();}
}
